...
function UpdateStaff() {
	...
	const [preview, setPreview] = useState(row?.photo);
	const [dob_date, setDobDate] = useState(row?.date_of_birth);
	const [employment_date, setEmploymentDate] = useState(row?.employment_date);
	const [attachFile, setAttachFile] = useState(row?.document_urls);
	...
	const handleSubmit = async () => {
		...
		if (
			...
			setErrTxts({ ...errTxts, emergency_name: 'Please enter a valid subject' });
			return;
		}
		if (form.emergency_phone) {
			if (
		...
		if (selectedFile) {
			...
			setIsLoading(true);
			const uplaod_file = update_file?.map((e, i) => {
				return uploadFile(e, getImageUrl(e));
			});
			const file_ = await Promise.all(uplaod_file);
			const new_file = file_?.map((e) => `${process.env.REACT_APP_S3_BASE_URL}${e}`);
			const previous_file = attachFile?.map((e) => e.url);
			if (!attachFile.length > 0) {
				delete data.document_urls;
			} else {
				data.document_urls = previous_file?.filter((url) => url).concat(new_file);
			}
			...
			<div className="add_staff_main">
				<div className="edit-staff-cont mx-auto">
					<div className="flex justify-between items-end">
						<span className="text-2xl self-end font-bold mr-28" style={{ fontSize: '20px' }}>
							<span className="">
								<IconButton
									onClick={() => {
										history.goBack();
					...
					<div className="bg-white rounded edit-staff-form-cont">
						<div className="edit-staff-form-heading">
							<span style={{ fontSize: '18px' }}>Staff Information</span>
						</div>
						...
						<div className="flex justify-center mt-32" style={{ gap: 20, marginTop: 50 }}>
							{isLoading ? (
