...
function AttachmentModal({ refresAttachments, row }) {
				...
				<FormControl variant="standard" className="w-5/12 student-select">
					...
					<Select
						...
						onChange={e => {
							...
							setCategory(value);
							if (value === 0) {
								setData({ ...data, category: 'Medical Form' });
							} else if (value === 1) {
								setData({ ...data, category: 'EpiPen Form' });
							} else {
					...
					>
						<MenuItem value={0}>Medical Form</MenuItem>
						<MenuItem value={1}>EpiPen Form</MenuItem>
