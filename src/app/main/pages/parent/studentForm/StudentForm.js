...
function StudentInformation(props) {
	...
	useEffect(() => {
		if (process.env.REACT_AP_ENV === 'dev') {
			// Incase of dev env, redirect all student forms to main website
			// Due to accidently sending email to real customers from dev env
			window.location.replace('https://perfectdaylive.com');
		}
		if (token) {
			axios
				.post('api/v1/parent/token', { token: `${token}==` })
				.then((res) => {
					JWTService.setSession(res.data.token);
					getStudent(res.data.student_id)
						.then((response) => {
							setStudent(response?.data);
							setHiddenCodes(
								response?.data?.parents.concat(response.data.approved_pickups)?.map((parent) => true)
							);
							setNotes({ student_exempt: response.data.student_exempt, notes: response.data.notes });
							setModifiedImmunizations(response.data.immunizations.filter((imm) => imm.is_enabled));
					...
					getImmunizations().then((resp) => setImmunizations(resp.data));
				})
				.catch((err) => {
					if (err.response.data.message) {
						...
						// 	})
						// );
						history.push({ pathname: '/expiredlink' });
					} else {
						dispatch(Actions.showMessage({ variant: 'error', message: 'Invalid token' }));
	...
	const goToEditMedications = () => {
		history.push({ pathname: '/medications', state: { student } });
	};
	...
	const goToEditStudent = () => {
		history.push({ pathname: '/studentformedit', state: { student } });
	};

	const goToAddEditContact = (edit, contact) => {
		history.push({ pathname: '/studentcontact', state: { isEdit: edit, row: student, contact } });
	};
	...
	const openAttachmentModal = () => {
		dispatch(
			Actions.openDialog({
				children: <AttachmentModal refresAttachments={getStudentForFile} row={student} />,
			})
		);
	};
					...
					<div className="P-m">
						<div className="stdinfo flex items-center flex-nowrap justify-between mx-auto">
							<span className="personal-hd info-hd stext-2xl self-end font-extrabold ">
								<h1 className="hd-main">Personal Information</h1>
							</span>
						...
						<div className="bg-white rounded p-32  mx-auto">
							<div className="flex">
									...
									<>
										...
										<div className="mt-20">
											<div className="bg-blue-100 px-40 pr-0 py-12 flex justify-between">
												<div className="font-bold med-table-headers mt-20 w-2/6">
													Medication
												</div>
												<div className="font-bold med-table-headers mt-20 w-2/6">Frequency</div>
												<div className="font-bold med-table-headers mt-20 w-2/6">
													Start Date
												</div>
												<div className="font-bold med-table-headers mt-20 w-1/6">End Date</div>
											</div>
											{student.is_medical_condition === 1 ? (
														...
														return (
															<>
																<div className="mt-10 px-40 pr-0 py-12 flex justify-between">
																	<div className="mt-20 w-2/6">{medication.name}</div>
						...
						<div className="P-m stdinfo flex justify-between mx-auto">
								...
								<div className="flex items-center flex-nowrap  mx-auto bg-white p-32">
									{isLoading ? (
										...
										<div className="room-detail">
											<h4>Homeroom</h4>
											<div
												className="text-2xl mt-2 font-bold capitalize"
												style={{ fontSize: 17 }}
											>
							...
							<div style={{ width: '48%' }}>
								<div className="flex items-center flex-nowrap justify-between mx-auto">
									<span className="text-2xl self-end font-extrabold ">
										<h1 className="hd-main">Attachments</h1>
									</span>
									<span>
										{student?.attachments.length === 5 ? (
											''
										) : (
											<div>
												<CustomButton
								...
								>
									{isLoading || isFileLoading ? (
										...
										<>
											{/* <div className="grid grid-cols-9" style={{ gap: '25px' }}>
											{!student.attachments.length ? (
												<div className="mt-10 flex justify-center items-center col-span-9 font-bold">
													{student.first_name} {student.last_name} has no attachments on
													record
												</div>
											) : (
												student.attachments.map((attachment, index) => (
													<>
														<div className="font-bold col-span-4">{attachment.name}</div>
														...
														return (
															<>
																...
																>
																	...
																	<div className="w-1/4">
																		{dayjs(new Date(attachment.expiry_date)).format(
																			'MMM DD, YYYY'
																		)}
																	</div>
						...
						<div className="contact-detail">
							...
							<div className="contact-table" style={{ overflowy: 'auto' }}>
								<TableContainer>
									<Table stickyHeader className={` table-1366x657`} aria-label="simple table">
										<TableHead>
											<TableRow>
												<TableCell className="bg-white school-table-header">Contact</TableCell>
												<TableCell className="bg-white school-table-header">Email</TableCell>
												<TableCell className="bg-white school-table-header">Phone</TableCell>
												<TableCell className="bg-white school-table-header">
													Emergency Contact
												</TableCell>
												<TableCell className="bg-white school-table-header" align="center">
													Can pickup
												</TableCell>
												<TableCell className="bg-white school-table-header action-div">
													Checkin code
												</TableCell>
												<TableCell className="bg-white school-table-header action-div">
													Action
																	...
																	<div className="grid grid-cols-2 auto-col-min auto-row-min">
																		<div className="flex items-center">
																			...
																			<div className="flex flex-col items-center">
																				<div className="form-parent-name truncate ">
																					{familie.first_name}{' '}
																					{familie.last_name}
																				</div>
																				<div className="form-parent-relation self-start truncate std-parent">
										...
										<div className="recomd notesrecord">
											{isLoading ? (
												...
												<div>
													...
													<h6 style={{ fontWeight: '700', fontSize: '17px' }}>
														{student?.student_exempt ? 'Yes' : 'No'}
													</h6>
													<br />
													<h4 style={{ fontSize: '18px', fontWeight: '700' }}>Notes</h4>
													{!isEditing ? (
														<TextareaAutosize
															disabled
															style={{
								...
								<table style={{ width: '100%' }}>
									...
									<tbody>
										...
										{student?.immunizations
											...
											.map((immu, i) => {
												const age = getAgeDetails(dayjs(student?.date_of_birth), dayjs());
